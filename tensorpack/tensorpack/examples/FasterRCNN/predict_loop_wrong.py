...
def smb_conn_sync():
    ...
    conn = SMBConnection(user_name, pwd, 'jetson-desktop', server_name, use_ntlm_v2 = True)
    assert conn.connect(server_ip, 139) # sharing server ip address, port 45 over http/ftp
...
if __name__ == '__main__':
    ...
    smb_conn_sync() #init local target_folder
    
    # clean up old images in result_folder_path && result_folder_path
    ...
    while True:
        time.sleep(1)
        do_compare()
        smb_conn_sync() # search new images
        num += 1
