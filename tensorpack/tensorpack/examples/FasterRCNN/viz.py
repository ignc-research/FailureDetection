...
def draw_final_outputs(img, results):
    ...
    for r in results:
        ...
        #tags.append("{}".format(cfg.DATA.CLASS_NAMES[r.class_id]))
    #ret = viz.draw_boxes(ret, boxes, tags)
    ret = viz.draw_boxes_texts(ret, boxes, tags) # draw_boxes_texts show failure labels in list at result image top left
    return ret
