...
def smb_conn_init():
    ...
    To adapt:
        share_folder, user_name, pwd, server_name, server_ip
    '''
    share_folder = 'myshare'
    user_name = 'mytest'
    pwd = 'mytest'
    #server_name = 'Legion'
    server_name = 'xp_server'
    server_ip = '192.168.0.10'
    local_dir = source_folder_path #'/home/jetson/xavier_page/source_folder/'
    
    #SMBConnection parameters: <username, pwd, client_name, server_name, use_ntlm_v2 = True>
    smb_conn = SMBConnection(user_name, pwd, 'jetson-desktop', server_name, use_ntlm_v2 = True)
    
    try:
        assert smb_conn.connect(server_ip, 139) # sharing server ip address, port 45 over http/ftp
    except:
        print('SMB connection error, please check network connection and server shares............')
    
    print('SMB connection established....')
    smb_conn.share_folder = 'myshare'
    smb_conn.local_dir = source_folder_path
    return smb_conn

def smb_conn_sync():
    '''
    work with function smb_conn_init()
    copy files from connected server to local
    To adapt:
        share_folder, user_name, pwd, server_name, server_ip
    '''
    #share_folder = 'myshare'
    #local_dir = source_folder_path
    filelist = smb_conn.listPath(smb_conn.share_folder, '/')
    ...
    for i in range(len(filelist)):
        if filelist[i].filename.endswith('.jpg'):
            ...
            with open(smb_conn.local_dir + '/' + filename, 'wb') as fp:
                #print(index, '.....copy:', filename)
                index += 1
                smb_conn.retrieveFile(smb_conn.share_folder,'/'+ filename, fp )
    #print('Files copied from share to /home/jetson/xavier_page/source_folder')
    

def do_compare():
    # print('Beacon ', num, 'Service Online, check in browser.!', f2, len(f2))
    #print('Beacon ', num, 'Service Online, check in browser.!', len(f2))
    system('clear') # clear terminal ouput
    
    #print('imges number in shared folder.....', len(f2))
    print(datetime.now().strftime("%H:%M:%S %d/%m/%Y"), 'Service running....')
    print()
    print('***Only the newest *5* images will be displayed in browser!')
    print()
    print('Server folder:', smb_conn.share_folder)
    print('Xavier folder:', smb_conn.local_dir)
    print()
...
if __name__ == '__main__':
    ...
    target = 'TM' # target file name contains string ***
    smb_conn = smb_conn_init() # build smb connection to server
    
    # clean up old images in source_folder_path && result_folder_path
    ...
    while True:
        time.sleep(1)
        do_compare()
        smb_conn_sync() # sync shared folder from server to local
        num += 1
